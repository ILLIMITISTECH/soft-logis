@extends('admin.layouts.admin')
@section('section')
    <div class="page-content">
        <!--breadcrumb-->
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            <div class="breadcrumb-title pe-3">Comptes</div>
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        <li class="breadcrumb-item"><a href="{{ route('admin.client') }}"><i class="bx bx-home-alt"></i></a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">Clients</li>
                    </ol>
                </nav>
            </div>
            <div class="ms-auto">
                <div class="btn-group">
                    <button type="button" class="btn btn-primary">Settings</button>
                    <button type="button" class="btn btn-primary split-bg-primary dropdown-toggle dropdown-toggle-split"
                        data-bs-toggle="dropdown"> <span class="visually-hidden">Toggle Dropdown</span>
                    </button>
                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end"> <a class="dropdown-item"
                            href="javascript:;">Action</a>
                        <a class="dropdown-item" href="javascript:;">Another action</a>
                        <a class="dropdown-item" href="javascript:;">Something else here</a>
                        <div class="dropdown-divider"></div> <a class="dropdown-item" href="javascript:;">Separated link</a>
                    </div>
                </div>
            </div>
        </div>
        <!--end breadcrumb-->

        <div class="card">
            <div class="card-body">


                <div class="table-responsive text-center">
                    <table class="table mb-0 table-striped table-bordered">
                        <thead class="table-light">
                            <tr>
                                <th>Logo</th>
                                <th>code</th>
                                <th>Identification</th>
                                <th>Raison sociale</th>
                                <th>email</th>
                                <th>Télephone</th>
                                <th>Etat</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($clients as $client)
                                <tr>
                                    <td>
                                        <img src='{{ asset('files/' . $client->logo) }}' class="product-img-2"
                                            alt="logo company">
                                    </td>
                                    <td>{{ $client->code }}</td>
                                    <td>
                                        <div class="d-flex align-items-center">

                                            <div class="ms-2">
                                                <h6 class="mb-0 font-14">{{ $client->identification }}</h6>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{ $client->raison_sociale }}</td>
                                    <td>{{ $client->email }}</td>
                                    <td>{{ $client->phone }}</td>
                                    <td>
                                        <span
                                            class="badge rounded-pill text-success bg-light-success p-2 text-uppercase px-3"><i
                                                class='bx bxs-circle me-1'></i>{{ $client->etat }}</span>
                                    </td>

                                    <td>
                                        <div class="d-flex order-actions">

                                            <a href="{{ route('admin.company.show', $client->uuid) }}" style="cursor: pointer" class="me-2"><i class="lni lni-eye"></i></a>

                                            <a data-bs-toggle="modal" data-bs-target="#editcompany{{ $client->uuid }}"
                                                style="cursor: pointer"><i class='bx bxs-edit'></i></a>

                                            <a class="ms-3 deleteConfirmation" data-uuid="{{ $client->uuid }}"
                                                data-type="confirmation_redirect" data-placement="top"
                                                data-token="{{ csrf_token() }}"
                                                data-url="{{ route('admin.client.destroy') }}"
                                                data-title="Vous êtes sur le point de supprimer {{ $client->raison_sociale }} "
                                                data-id="{{ $client->uuid }}" data-param="0"
                                                data-route="{{ route('admin.client.destroy') }}"><i class='bx bxs-trash'
                                                    style="cursor: pointer"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                {{-- Modal edit  --}}
                                <div class="modal fade" id="editcompany{{ $client->uuid }}" tabindex="-1"
                                    aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-scrollable">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Modifier client</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </div>
                                            <form method="post"
                                                action="{{ route('admin.client.update', $client->uuid) }}"
                                                class="submitForm" enctype="multipart/form-data">
                                                @csrf
                                                <div class="modal-body">

                                                    <div class="form-group">
                                                        <label for="logo" class="col-form-label">logo:</label>
                                                        <input type="file" class="form-control" id="logo"
                                                            name="logo" >
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="identification"
                                                            class="col-form-label">Identification:</label>
                                                        <input type="text" class="form-control" id="identification"
                                                            name="identification" value="{{ $client->identification }}" >
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="raison_sociale" class="col-form-label">Raison
                                                            Sociale:</label>
                                                        <input type="text" class="form-control" id="raison_sociale"
                                                            name="raison_sociale" value="{{ $client->raison_sociale }}">
                                                    </div>
                                                    <div class="row">
                                                        <div class="form-group col">
                                                            <label for="email" class="col-form-label">Email
                                                                Entreprise:</label>
                                                            <input type="email" class="form-control" id="email"
                                                                placeholder="exemple@example.com" name="email"
                                                                value="{{ $client->email }}">
                                                        </div>
                                                        <div class="form-group col">
                                                            <label for="phone" class="col-form-label">Télephone
                                                                Entreprise:</label>
                                                            <input type="phone" class="form-control" id="phone"
                                                                placeholder="+123456789" name="phone"
                                                                value="{{ $client->phone }}">
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="localisation"
                                                            class="col-form-label">Localisation:</label>
                                                        <input type="text" class="form-control" id="localisation"
                                                            name="localisation" value="{{ $client->localisation }}">
                                                    </div>

                                                    <div class="form-group">
                                                        <label for="description"
                                                            class="col-form-label">Description:</label>
                                                        <textarea name="description" id="description" cols="30" rows="3" class="form-control"
                                                            value="{{ $client->description }}"></textarea>
                                                    </div>

                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary"
                                                            data-bs-dismiss="modal">Fermer</button>
                                                        <button type="submit" class="btn btn-primary">Modifier</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                                </div>

                            @empty
                            <tr>
                                <td colspan="8" class="text-center">Aucun client enregistré</td>
                            </tr>

                            @endforelse


                        </tbody>
                    </table>
                </div>
            </div>
        </div>


    </div>

    {{-- Modal --> --}}

    <!-- Modal add Company-->
    <div class="modal fade" id="addcompany" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Ajouter Client</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form method="post" action="{{ route('admin.client.store') }}" class="submitForm"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">

                        <div class="form-group">
                            <label for="logo" class="col-form-label">logo:</label>
                            <input type="file" class="form-control" id="logo" name="logo" autocomplete="off">
                        </div>
                        <div class="form-group">
                            <label for="identification" class="col-form-label">Identification:</label>
                            <input type="text" class="form-control" id="identification" name="identification" autocomplete="off">
                        </div>
                        <div class="form-group">
                            <label for="raison_sociale" class="col-form-label">Raison Sociale:</label>
                            <input type="text" class="form-control" id="raison_sociale" name="raison_sociale" autocomplete="off">
                        </div>
                        <div class="row">
                            <div class="form-group col">
                                <label for="email" class="col-form-label">Email Entreprise:</label>
                                <input type="email" class="form-control" id="email"
                                    placeholder="exemple@example.com" name="email" autocomplete="off">
                            </div>
                            <div class="form-group col">
                                <label for="phone" class="col-form-label">Télephone Entreprise:</label>
                                <input type="phone" class="form-control" id="phone" placeholder="+123456789"
                                    name="phone" autocomplete="off">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="localisation" class="col-form-label">Localisation:</label>
                            <input type="text" class="form-control" id="localisation" name="localisation" autocomplete="off">
                        </div>

                        <div class="form-group">
                            <label for="description" class="col-form-label">Description:</label>
                            <textarea name="description" id="description" cols="30" rows="3" class="form-control" autocomplete="off"></textarea>
                        </div>
                        {{-- <hr class="my-3"> --}}

                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fermer</button>
                            <button type="submit" class="btn btn-primary">Sauvegarder</button>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>
    {{-- </div> --}}
@endsection
